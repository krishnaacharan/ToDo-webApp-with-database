<?php
    ...
    if(isset($_POST['submit'])){
       ...
       mysqli_query($db,"INSERT INTO tasks2(task) VALUES ('$task')");
    ...
    if(isset($_GET['del_all'])){
        mysqli_query($db,"DELETE FROM tasks2");
        header('location:index2.php');
    }
    /*(isset($_GET['update_task'])){
        $id=$_GET(['update_task']);
        mysqli_query($db,"UPDATE tasks2 SET task=$updatedinfo WHERE id=$id");
    }*/
?>
...
<body>
    ...
    <div nav></div>
        ...
        <div class="container animate__fadeInLeft">
            <div class="top">
               <a href="index2.php?del_all"> <i class="fa fa-refresh" ></i></a>
               <h1>Things to At Gym</h1>
            </div>
            <div class="content">
            <span class="unchecked"id=-1 style="color: white">Added Items</span>
                <?php $i=1;while($line=mysqli_fetch_array($tasks2)){?>
                    <div class="lines">
                           <label style="color: white" for="done"class="omg<?php echo $line['id']?> fa fa-circle-thin" onclick=correct(<?php echo $line['id']?>)></label>
                            ...
                            <!--<div class="edit fa fa-save" style="display:contents"onclick=edited()><a href="index2.php?update_task="></a></div>-->
                            <div class="dele"><a href="index2.php?del_task=<?php echo $line['id'];?>">Remove</a></div>
                    </div>
